...
class ResponseStreamCoordinator:
    ...
    def intercept_event(
        ...
        with self._lock:
            ...
            #     event.node_run_result.outputs.items():
            #     self._variable_pool.add(
            #         (event.node_id, variable_name), variable_value
            #     )
            return self.try_flush()
        ...
        while self._has_unread_stream(segment.selector):
            if event := self._pop_stream_chunk(segment.selector):
                ...
                if (
                    ...
                    and source_selector_prefix not in self._graph.nodes
                ):
                    response_node = self._graph.nodes[self._active_session.node_id]
        ...
        with self._lock:
            ...
            while self._active_session.index < len(template.segments):
                ...
                if isinstance(segment, VariableSegment):
                    ...
                    # Only check for actual nodes, not special selectors
                    # (sys, env, conversation)
                    source_selector_prefix = (
                        segment.selector[0] if segment.selector else ""
                    )
    ...
    def loads(self, data: str) -> None:
        ...
        with self._lock:
            ...
            self._closed_streams = {
                tuple(selector) for selector in state.closed_streams
            }
